...
public class ManagerCard : MonoBehaviour {
    ...
    public void BackToRandom() {
        for (int i = 0; i < selectCardMenu.cardProperties.parentCardPrefab.transform.childCount; i++)
        {
          
            Destroy(selectCardMenu.cardProperties.parentCardPrefab.transform.GetChild(i).gameObject);
           
        }
        selectCardMenu.cardProperties.cardinMenu.Clear();
        selectCardMenu.cardProperties.cardinMenuGameobject.Clear();
        MenuRandom.SetActive(true);
        MenuChooseCard.SetActive(false);
    }
}
